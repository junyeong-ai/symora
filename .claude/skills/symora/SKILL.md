...
4. `symora map file <path>` — compact file overview. `backend` says what read the declarations; `focus_symbols` and `backend` are omitted together only when neither producer could (an empty list would say the file declares none).
5. `symora symbols <file>` — the file's declarations, from the language server when it serves and the compiled-in grammar when it does not; `backend` (`document` | `ast`) says which. A `document` answer nests and carries the server's kinds; an `ast` answer is flat and carries containment in `name_path`. `--body`/`--signature` work either way. Or `symora symbols --symbol <path>`/`--name <name>` with no file: index-backed workspace resolution that finds methods too.
...
- Reading an `impact` verdict: `confidence` reaches 1.0 only when the caller walk both found callers and exhausted the graph, so anything lower makes the counts a floor (0.8 = stopped at `max_depth_reached` or the fan-out cap, 0.5 = no direct callers at all). `risk` escalates on `is_exported`, which is published only for languages whose compiler decides visibility (Rust, Go, Java, Kotlin, C#, TypeScript, JavaScript); elsewhere it is absent and the label follows the caller counts alone.
- A zero reference count on a METHOD or other member is not unreachability — a call reaches a member through the type that declares it, and `hints` names that type and how often it is used. A free function's zero is reachability, and stands bare.
...
- `search symbols` returns `count: 0` with a hint naming a language: the zero is not exhaustive for that language — either it has no index extractor and no live language server, or the search ran workspace-only (index not built, or `--workspace-symbols`) and the live lookup failed, or enough matches came from other languages that this one was never searched. `next_commands` carries the remedy that fits the cause: `search content`/`doctor`, `search index build` when building the index can actually cover the language, re-running without `--workspace-symbols`, or re-running with `--lang <language>` for one that was never searched. A bare `count: 0` — no hint, no `coverage_gaps` — is the strongest answer this tool gives: every requested language was in the index's build scope and disk agreed, so the zero really is "nothing exists". Use it to confirm a rename left nothing behind, or that a declaration is gone; nothing weaker needs `git grep` beside it. It is not an "unused" check — that asks about references to a declaration that exists, which is `usage --filter zero-refs`, read with the member rule above.

  Two things weaken it, and they are different axes. `coverage_gaps` names a language outside the answer's domain — the index never covered it and its live lookup failed too — so the zero says nothing about that language. A hint saying the index is "behind the working tree" means the opposite: the language IS covered, but files have changed since the build and no language server re-read them, so a symbol written since then has no row to match. Run the `symora search index build` in `next_commands` and re-run the query; the zero is then bare.

  A bare zero does not require a language server. The index's scope answers which languages it speaks for, and a stat-only walk answers whether the tree still looks the way the build read it — so an environment with no servers installed at all still gets authoritative zeroes, as long as the index is built.
