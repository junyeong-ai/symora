...
- `search symbols` returns `count: 0` with a hint naming a language: the zero is not exhaustive for that language — either it has no index extractor and no live language server, or the search ran workspace-only (index not built, or `--workspace-symbols`) and the live lookup failed, or enough matches came from other languages that this one was never searched. `next_commands` carries the remedy that fits the cause: `search content`/`doctor`, `search index build` when building the index can actually cover the language, re-running without `--workspace-symbols`, or re-running with `--lang <language>` for one that was never searched. A bare `count: 0` — no hint, no `coverage_gaps` — is the strongest answer this tool gives: every requested language was in the index's build scope and disk agreed, so the zero really is "nothing exists". Use it as a rename-completeness or dead-code check; nothing weaker needs `git grep` beside it.
  ...
  Two things weaken it, and they are different axes. `coverage_gaps` names a language outside the answer's domain — the index never covered it and its live lookup failed too — so the zero says nothing about that language. A hint about confirming "against the working tree" means the opposite: the language IS covered, but nothing re-read the tree, so the zero is the last build's and a symbol written since then has no row to match. Its remedy is the `search content` command in `next_commands`, which reads the files directly; `search index build` makes the next zero authoritative.
