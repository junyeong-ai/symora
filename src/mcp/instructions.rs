...
5. Aggregate: `get_context` returns callers, callees, types, and tests in one call — prefer it over four separate queries. For flow questions, set with_bodies=true: complete target and callee bodies arrive in the same response under a body_tokens budget; bodies_included on a section says how many items carry one — fetch any missing body with `inspect_symbol` instead of reading the file.
