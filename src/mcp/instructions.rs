...
4. Navigate from a symbol position. The graph queries — `find_references`, `find_callers`, `find_callees`, `find_implementations`, `find_call_path` — take file:line[:column], omitting the column to address the symbol declared on the line; `find_call_path` answers \"how does function A reach function B\" with the ordered call chain, or an honest reachability verdict that never overclaims. `find_definition` and `get_hover` resolve at the exact file:line:column — pass the symbol's column.
