...
2. Locate: `search_symbols` when you only know a rough name; `search_content` for keywords and strings. `get_index_status` says what a completed build covers, and `build_index` is what a symbol zero's hints prescribe when it does not — a symbol zero carrying no hint and no coverage_gaps is the one that means nothing exists. `search_content` without a language reads code files only, so documentation and configuration formats are reached by naming one; it falls back to the working tree for whatever the index does not hold, which makes its zero confirmed against disk rather than a stale index's guess. `search_ast` is the structural route for a language the index and the servers do not cover, and for forms a name cannot express.
3. Inspect: `get_file_overview` before reading any file in full; `list_file_symbols` for one file's symbols — backend says which producer read it, document (nested) or ast (flat, containment in name_path), and both answer without the file being read in full; `inspect_symbol` to resolve a path like Class/method from a search hit.
...
6. Before changing a symbol: `get_impact` reports test/prod reference counts, affected files, and a risk-ranked caller graph. Read the counts, not only the label: confidence reaches 1.0 only when the caller walk both found callers and exhausted the graph, and a zero reference count on a member is not unreachability — a call reaches it through the type that declares it, which the hints name.
